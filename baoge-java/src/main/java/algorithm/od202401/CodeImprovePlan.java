 ...
 * 首先，我们需要定义一个函数 check，该函数接受一个整数 x作为参数，并返回一个布尔值。这个函数的作用是检查是否可以使用数值 x将数组分割成M份，
 * 使得每份的和都不超过 x。如果 x足够大，那么肯定可以完成这样的划分；但如果 x太小，则无法满足条件。因此，我们需要找到最小的可行的 x。
 *
 * 一种常见的方法是贪心算法。我们可以从数组的第一个元素开始，依次将每个元素放入当前子数组中，同时计算子数组的和。
 * 当我们遇到一个元素使得加入后子数组的和超过了限制 x时，就开始一个新的子数组。这样，我们可以保证每个子数组的和都不会超过限制。
 *
 * 另外，为了优化算法的效率，我们可以使用动态规划的方法。我们可以维护一个长度为 n+1的数组 dp，其中 dp[i]表示前 i个元素被划分为M份时能够达到的最小最大值。
 * 初始时，dp[0] = 0，而其他元素 dp[i] = float('inf')。然后，我们遍历数组中的每个元素，更新 dp数组。
 * 具体地，对于每个元素，我们可以选择将其放入已有的某一份中或者新建一份。在更新过程中，我们需要比较这两种选择的结果，取其中的较小值来更新对应的位置。
 * 最终，dp[n]就是我们要求的答案，即最小的最大值。
 */
