 ...
 * ==================================================
 *
 * 在JDK 1.5之前，我们若想实现线程同步，只能通过synchronized关键字这一种方式来达成；底层，Java也是通过synchronized关键字来做到数据的原子性维护的；
 * synchronized关键字是JVM实现的一个内置锁，从底层角度来说，这种锁的获取和释放都是由JVM帮助我们隐式实现的。
 *
 * 在JDK 1.5开始，并发包引入了Lock锁
 * Lock同步锁是基于Java来实现的，因此锁的获取与释放都是通过Java代码来实现与控制的；然而synchronized是基于底层操作系统的Mutex Lock来实现的，
 * 每次对锁的获取与释放动作都会带来用户态与内核态之间的切换，这种切换会极大地增加系统的负担；在并发量较高时，也就是说锁的竞争比较激烈的时候，
 * synchronized锁在性能上的表现就非常差【这也是很多人认为使用synchronized关键字的性能不太好的原因】。
 *
 * 但是！！从JDK1.6开始，synchronized锁的实现发生了很大的变化；JVM引入了相应的优化手段来提升synchronized锁的性能，
 * 这种提升涉及到偏向锁、轻量级锁、重量级锁等，从而减少锁的竞争所带来的用户态与内核态之间的切换；
 * 这种锁的优化实际上是通过Java对象头【这个在JVM的学习中已经学习过了】中的一些标志位来去实现的；对于锁的访问与改变，实际上都与Java对象头息息相关。
 *
 ...
 * 1、无锁标记
 *
 * 2、偏向锁标记（针对一个线程来说，多次访问同一个对象的锁）
 *
 * 3、轻量级锁标记（自旋就是一种轻量级锁实现方式）
 *
 * 4、重量级锁标记【这就是通常意义上我们理解的synchronized的情况，认为它是很重的，从用户态转向内核态】
 *
 * 5、GC标记
 *
 * 对于synchronized锁来说，锁的升级主要是通过Mark Word中的锁标志位与是否是偏向锁标志位来达成的；
 ...
 * 无锁  ->   偏向锁  ->   轻量级锁  ->   重量级锁
 *
 * 偏向锁(适用于一个线程访问同步代码块的场景)：
 * 它是针对一个线程来说， 它的主要作用就是优化同一个线程多次获取一个锁的情况；如果一个synchronized方法被一个线程访问，
 * 那么这个方法所在的对象就会在其Mark Word中将偏向锁进行标记，同时还会有一个字段来存储该线程的ID;当这个线程再次访问一个synchronized方法时，
 * 它会检查这个对象的Mark Word的偏向锁标记以及是否指向了其线程ID，如果是的话，那么该线程就无需再去进入管程（Monitor）了，
 * 而是直接进入到该方法体中。
 * 如果是另外一个线程访问这个synchronized方法，那么实际情况会如何呢？偏向锁会被取消掉。
 *
 * 轻量级锁：
 * 若第一个线程已经获取到了当前对象的话，这时第二个线程又开始尝试争抢该对象的锁，由于该对象的锁已经被第一个线程获取到，
 ...
 *
 * 自旋锁(上述Spin也有解释)：
 * 若自旋失败（依然无法获取到锁），那么锁就会转化为重量级锁，在这种情况下，无法获取到锁的线程都会进入到Monitor（既内核态）。
