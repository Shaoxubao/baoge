 ...
 * -XX:PretenureSizeThreshold=4194304(4M)------>当创建对象大小超过指定阈值，直接进入老年代分配内存,搭配串行垃圾收集器使用（-XX:+UseSerialGC）
 *
 */
public class MyTest2 {
    ...
    public static void main(String[] args) {
        ...
        byte[] myAlloc = new byte[5 * size]; // 直接进入老年代,若改成8M,不是用-XX:UseSerialGC参数，也会直接进入老年代，因为eden区容纳不了,
                                             // 若改为10M，则出现对内存溢出异常
